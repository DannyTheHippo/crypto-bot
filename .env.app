...
# ~$2.2-2.5/day at current skip rates; fallback = drop LINK on sustained >$3/day.
...
AGENTIC_MAX_CALLS_PER_DAY=700 # DailyLlmBudget cap on LLM calls per UTC day (500→700 at the 5-symbol widening: 480 bar-opportunities/day must not brush the cap under prescreen fail-open)
...
AGENTIC_PLAYBOOK_AB_PCT=25 # W4.1 champion/candidate A/B LIVE (owner 2026-07-08): % of decides routed to the newest INACTIVE reflection candidate for per-version attribution. 50→25 (loop-domain, 2026-07-13 Pass 21): the 50% premise is falsified — v2 entered on 0 of 17 FLAT-state consults since mint (v1: 16/57; P≈0.4% under v1's rate), so serving share cannot create candidate evidence for an abstaining candidate; it only halves the champion entry stream that feeds the symmetric floor, reflection cadence, and the info-context A/B; 0 disables
