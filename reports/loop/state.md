...
| 35 | Promotion-walk LINK freeze + stuck RECONCILE_REQUIRED order (found Pass 17): the walk carries a +6.92 LINK (~$55) phantom open — our journaled fills genuinely net +6.92 (8 distinct venue-native trade ids, no double-ingestion) while portfolio + clean reconciliation say 5.62+dust. Mechanisms: (a) order `cbt019f4e87283f743eb57413f15c951bb7` (BUY 5.65 LINK, 07-11 00:19Z) stuck non-terminal RECONCILE_REQUIRED with cum_qty=1.67 vs 3 journaled partials summing 5.65 — also keeps `hasUnresolvedOrders()` true (live-arming blocker, #25 class); (b) shared demo wallet: foreign traffic moves the balance without entering our fills. Consequence: NO LINK round trip counts on the promotion gate (or reaches reflection evidence) while the offset persists. Fix = OMS/reconciliation territory (terminalize via journaled reconcile adoption; and/or bounded stray-offset reset in the walk). SQL in LOG.md Pass 17 | 2 | M | FLAGGED 2026-07-12 Pass 17 — owner/OMS territory, report-only; WATCH: if the gate-RT vs in-memory gap grows beyond the two explained classes (XRP epoch phase-shift = count-preserving; LINK freeze), that is the §7 measurement-trust stop condition |
...
- **MEASUREMENT 2026-07-12 (Pass 17) — promotion-walk undercount, one frozen symbol.** Gate RT
  added +2 today where +4 closed: BTC/SOL clean, XRP phase-shifted (documented epoch-straddle
  class, count-preserving long-run), **LINK frozen — see backlog #35** (stuck
  RECONCILE_REQUIRED order + shared-wallet foreign drift ⇒ fills-walk and balance truth cannot
  reconcile; LINK evidence silently stopped accruing). Not the §7 trust breach yet — both
  divergence classes are explained and conservative — but the watch is armed (#35).
