...
else is looked up on demand; § Index says which file answers which question. Last updated **Pass 59,
2026-08-03**. Hard cap 200 lines — anything longer belongs in the file that owns it, with a pointer here.

## ⛔ THE PERP TRADES AXIS IS DEAD — a BLOCKED defect, and the FIRST item for the next pass

> **VERIFIED twice with positive controls:** over 13h `binanceusdm` `sum(trades_checked)` = **0** across
> 777 passes while `binance` = **6,356** across 778; and **zero `FILL_FOR_UNKNOWN_ORDER`** halts exist in
> 32,579 `audit_log` rows. **The reconciler's fill-backfill AND its `FILL_FOR_UNKNOWN_ORDER` corruption
> detector are inert on the venue holding every current position.** Hypothesis **UNPROVEN**: checkpoint
> seeds to `0`, advances only inside the trades loop, and USD-M `fapi/v1/userTrades` bounds startTime to
> 7 days (empty at `0`) while spot `myTrades` does not. **Blocked on the #54 keyed in-container probe**;
> the fix reactivates a dormant HALTING detector on the live money path — ship observation-only first.

## ⚠ `loop:sweep` REPORTS 1 alarm (was 2). It is EXPECTED — but its recorded clearing path is UNREACHABLE

> **`venue_reject_rate_high [binance]` — 16/20 = 80.0%. Do not investigate, do not tune.** All 20 submits
> predate `f5abf8a`; all 16 rejects are one `reduce_only SELL ZEC/USDT` retried half-hourly. **Do not
> lower the threshold** (binanceusdm 4/186, Wilson 99.9% UB 8.45%, spec-pinned). It will NOT clear on a
> spot entry — it **ages out ~2026-08-07T01:45Z** into `venue_reject_rate_undetermined`, silence not
> health. Re-verified Pass 59: newest binance submit **2026-07-31T01:45:02Z**.
>
> **`venue_reject_rate_high [binanceusdm]` — STOPPED FIRING (0/20), REPAIR STILL NOT SHIPPED.** A cleared
> alarm is not a fixed defect: the `-4024` `PERCENT_PRICE` frame mismatch recurs on the next perp exit
> priced against the live feed. The obvious fix was REFUTED; the correct seams need
> `agentic.strategy.ts:1434-1438` first. `incidents/2026-07-31-perp-exit-band-rejects.md`.

## ⚠ Six standing cautions — bodies live elsewhere; these are the facts, follow the pointer

> **THE LANE IS WORSE THAN DOING NOTHING.** `BELOW_PASSIVE_BENCHMARK` fires on **evidence**: 28-asset
> basket **−2.175%** ⇒ **≈$37 of the loss was strategy, not beta**. **Four root-caused findings remain
> UNSHIPPED** — `incidents/2026-08-01-spurious-unknown-ours-halt.md`. **The through-line has now held
> four passes** (`config_snapshots`, `fee_ledger`, Pass 58's alarm text, and Pass 59's `error_class`
> label + WATCH-V4-1 halt claim + V10-1 comparator): _a surface reporting health it never established._
>
> **`entryVwap` IS BUY-SIDE ONLY ⇒ the anchor is the COVER price on every SHORT trip**; biases Arm2/Arm3,
> **not fixed on purpose**. `studies/frame-audit-2026-08-03.md`.
>
> **THE HORIZON GRID FLATTERS EVERY RESULT** — the gap to the +13.0 bar is **~70–125 bps, not ~30**.
> **Re-read any prior finding before quoting it.** `verdicts.md`; `learning-capacity-2026-07-31.md`.
>
> **TWO LIVE BEHAVIOUR CHANGES SHIPPED 2026-07-30, NEITHER CREDITABLE ALONE** — v10 `inverted` (**never
> quote +47.6 as an edge**) and `AGENTIC_PLAN_AUTHORITATIVE_EXITS=true`, same boot, no control arm.
>
> **THE LLM LANE IS FUNDED — do not investigate a latch** (`agent_client_latch_cause` all-zero Pass 59;
> any cause but `insufficient_credit` IS an incident). **`POSITION_DRIFT` HAS NEVER HALTED THIS SYSTEM** —
> all 18 `RECONCILE_MISMATCH` halts in 32,579 `audit_log` rows are `UNKNOWN_OURS_OPEN`, and a `streak>=2`
> debounce (`1ff1fc7`) means a halt needs two CONSECUTIVE divergent passes. `watches.md` § WATCH-V4-1.

## Current order & status

- **Live build `ea68379`**, boot 17:00:03Z, healthy, `RestartCount` 0, health 200/200, kill switch
  RUNNING, latch causes all zero. Redeploys re-seed the `agentic_venue_stop_total` counters
  WATCH-V4-11/V4-13 read, and reset RSS. **Promtool trap** (in-container check after a host edit is
  VOID): playbook §5.3. **On a fresh boot `reconciliation_last_success_timestamp_seconds 0` and
  `agentic_budget_remaining_usd 0` are the documented carve-outs, not faults** — clean stamp is aged
  from `StartedAt`, budget gauge is inside the 5-min `BUDGET_GAUGE_INIT_GRACE_MS`.
- **A $0.61/day TRUNCATION LEAK IS WIRED FLAG-OFF** (`ea68379`) — `AGENTIC_OUTPUT_EFFORT`, optional enum,
  NO default ⇒ request byte-identical until enabled. **The enable is a SEPARATE commit + $0 offline
  review** (`charter.md`). Detail and the two refuted alternatives: **WATCH-V4-12** row below.
- **`config_snapshots` HAS A WRITER NOW** (`118132c`). Writer FAILS OPEN, reader FAILS CLOSED. **WATCH-V4-16.**
- **THE PIN-SET LEAK IS STILL OPEN AND WAS NOT REACHED** — Pass 59's defect load consumed it. ≈$0.33/day;
  repair withdrawn (a "was once real" guard the `PortfolioViewPort` seam cannot reconstruct); **sound
  route = a durable round-trip-cycle reader**. `LOG.md` § Pass 58.
- **THE SPOT LANE IS SEVERELY SUPPRESSED, AND THE OBVIOUS EXPLANATION IS HALF WRONG.** `sideEligibility`
  is **prompt payload, not a code gate**. Real: 191 spot consults → **0** entries, P(0) ≈ 3.8e-4 —
  **confounded with v10**. `verdicts.md` BINDS: _"Do not propose cost work as a profitability lever."_
  Backlog 58, expectancy-framed. **Pass 59: still zero binance submits — newest is 2026-07-31T01:45Z.**
- **`llm_usage` IS NOT A DEFECT — vestigial by design** (writer deleted in `9a63edf`); **`llmCostUsd` is
  CORRECT**. **Do not drop those 69 rows** — inside the epoch, removing them loosens a permission gate.
- **The playbook documents all 22 alarm kinds** (`7e1306c`); **`loop-sweep-specs` IS on the production
  gate** (`6149861`). Run `loop:harness` (~15s) on `harness_stale`.
- **`loop:authoring`: an API-shape failure burns the whole UTC day.** `WATCH-DEPLOY-HALVES-1` sample zero.
- **THE BAR THIS PROGRAM GATES ON WAS NEVER DERIVED** — +13.0/+24.2 enter the repo fully formed in
  `7b3e977`; every later citation is circular. Demo cost 9.29 bps/trip. `fee-floor-derivation-2026-07-31.md`.
- **THE `−16.9 bps` ENTRIES FIGURE IS WRONG — it is `−13.75` (n=64, not 61)**; **five `verdicts.md`
  citations stale**; mirror UNVERIFIED beyond h=1.
- **Cost shape:** LLM runs AT the $3/day breaker (~$0.123/h ⇒ projects ~$2.96). **The timing knobs, not
  the model, set the bill.** The `charter.md` `$5/day` drift is **RESOLVED Pass 59**.
- **FAMILY B IS NOT BLOCKED — the 2026-07-31 drift finding was WRONG** (genuine NUL separator). **Lesson:
  a reimplemented hash is a second source of truth — import it or do not compute it.**
  `studies/corpus-fingerprint-drift-correction-2026-08-03.md`.
- **The book at 2026-08-03T16:07Z, as ONE atomic tuple** (single `evaluate()` sample): `roundTrips=48,
  windowDays=10.85, netPnlUsd=−60.34, llmCostUsd=28.15, winRate=0.25, ready=false,
  reasons=[NON_POSITIVE_NET_PNL, INSUFFICIENT_WINDOW, BELOW_PASSIVE_BENCHMARK]`; `equity_usdt` 4966.77,
  RUNNING. **Never quote these from separate reads.** Over Pass 58→59 (6.37h): +2 trips, netPnl −$0.41,
  LLM +$1.19 ⇒ **realized-minus-fees moved +$0.78 while the lane billed $1.19** — noise at this window
  length, but the first read in weeks where gross was not the negative term. **Gross is −$32.19 lifetime,
  so zero LLM spend still cannot make the book positive.**
- **WATCH-PLAYBOOK-V10-1 FIRED POWERED AND IS UNADJUDICABLE AS WRITTEN — do not re-derive; ROLLBACK
  REFUSED.** `AGENTIC_PLAYBOOK_PIN=8` re-arms v9+v10, activates Thompson sampling for the first time
  ever, and silently cancels the daily-minting override. **The sweep's adverse-selection annotation is
  FALSE for this metric** — text fix NOT yet shipped. Full reasoning: the WATCH row below; `LOG.md` § 59.
- **THE PROMOTION GATE IS NOT REACHABLE ON THIS EDGE — arithmetic, not opinion.** 48 trips vs a floor
  of 30, so **the WINDOW binds**: 14 days not met before **08-06 18:00Z**. Gross is negative ⇒ **even
  zero LLM spend cannot make it positive**. `success-exit-2026-07-31.md`.
- **Backlog 56 is REAL and Pass 59 proved it again**: `test:cov` is NOT on the green path, so a new
  branch shipped at **99.5%** against the mandated 100% for `src/features/trading/execution/**` and only
  the adversarial reviewer caught it (fixed, now **795/795**). Run it on any execution/risk/oms diff.
...
- **COLLISION #6 (Pass 58)** — an owner-directed INTERACTIVE session ran inside Pass 58's lease and took
  the number `57`, declaring _"No lease taken"_: the lease binds only callers. Zero file overlap.
  **Stage only files the pass authored — `git add -A` would have committed its in-flight work.**
  Pass 59: no collision, tip re-verified `a23141a` before staging. `LOG.md` § Pass 58.
- **`loop:fanout declare` OVERWRITES A LIVE ROSTER** with no refusal and no versioning — Pass 59's
  second declare destroyed the read-only fan-out's denominator, so `join` reported four returned lanes
  as _"never declared"_. Declare ONCE per pass, or join before re-declaring. `LOG.md` § Pass 59.
- **Last pass:** Pass 59, 2026-08-03 (`LOG.md`). Cadence 3×/day; take the lease before any edit, release
...
| id | expected-positive | status at Pass 59 |
| --- | --- | --- |
...
| WATCH-PLAYBOOK-V10-1 | live entry forward return under v10 lands above `champion_v8`'s replayed −12.7/−36.3/−32.7/−70.1 bps | **FIRED POWERED Pass 59 (n=21, k=8) — and UNADJUDICABLE AS WRITTEN: the clause is a CATEGORY ERROR (live cell vs replay cell).** Do not re-derive; restate against the live-vs-live v8 cell the tool already computes (`REPLAY_REFERENCE.incumbent` is loaded and never rendered). **ROLLBACK REFUSED** — see the § Current order entry. Established: `inverted` did not reproduce out of sample at h=4/h=8, and v10's entries are worse than the market it traded (powered) |
...
- **THE OWNER DECISION THIS PROGRAM TURNS ON** — the gate CANNOT be passed, by arithmetic: an arm must
  ...
  count, universe **28 BASES not 40 strings**. `success-exit-2026-07-31.md` — "STOP with extra steps"
  unrebutted.
- **GO/NO-GO OWED: the ~$22 Family B paid edge run** — blocker refuted 08-03, the paid block is atomic.
  **Not run unilaterally**; the deployment's own falsification test.
- **THE OOS SESSION ARM HAS NEVER RUN** — shipped Pass 57 (`ea95642`, 3,650 lines), no sealed window; its
  own § Cadence says the **hourly trigger is OWNER-SIDE**. `studies/oos-session-arm-2026-08-03.md`.
- **ONE `.env.app` edit hook-blocked** — `:153` spot/`STOP_LOSS_LIMIT` FALSE since `f5abf8a`. **`:159`
  ...
  **The daily mint has produced ZERO candidates in 3 days** (one row, id 16, 08-01); 08-03 slot UNSPENT.
