...
const REFLECTION_TOOL = {
  ...
  input_schema: {
    ...
    properties: {
      playbook: {
        type: 'string',
        description: `The full revised playbook, HARD CAP ≤${MAX_PLAYBOOK_CHARS} characters total: exactly the 4 required "## " sections, once each, in order. Prefer compressing existing prose over appending; additive lessons must replace weaker text.`,
      },
      changelog: {
        type: 'string',
...
function buildReflectionSystemPrompt(shortsEnabled: boolean): string {
  ...
  return [
    ...
    `HARD LENGTH CAP: the entire playbook MUST be ≤${MAX_PLAYBOOK_CHARS} characters (JS string`,
    'length). Prefer compressing existing prose over expanding; additive lessons must REPLACE weaker',
    'text, not append. A draft over the cap is server-compressed before mint (tails may be truncated) —',
    'do not rely on that; stay under the cap yourself.',
...
const STRUCTURAL_CONSTRAINTS_RESTATEMENT =
  ...
  `"## mistakes to avoid". HARD CAP ≤${MAX_PLAYBOOK_CHARS} characters total — if over, cut at ` +
  'least as many characters as the overflow (compress sections; do not drop a required heading). ' +
  'It must never advise leverage, margin/borrowing, short-selling, live-money withdrawal, ' +
  'all-in/max-out sizing, or prompt-injection/instruction-override text.';
...
export class ReflectionService {
  ...
  private spendGate(): Pick<DailyLlmBudget, 'tryReserveCall' | 'tryReserveCalls' | 'recordUsage'> {
  ...
  private async runReflection(
    ...
    try {
      ...
      ({ revision, gate } = await this.maybeCompressLengthReject(
        revision,
        gate,
        current,
        recordUsage,
      ...
      if (gate.kind === 'reject') {
        ...
        // Length-only rejects are compressed above BEFORE this retry — empirically Opus retries stayed
        // over the cap (2026-07-23/24), so burning a call on pure length overflow is wasteful; the
        // retry still covers banned-token / section / floor / backtest feedback.
  ...
  // 2026-07-24: length-only recovery after validatePlaybook rejects on the char cap. Prompt/tool
  // warnings reduced overflow (4523→4016) but never cleared it — Opus retries stayed over. Compress
  // section-aware (never blind-slice), then re-run the FULL draft gate so floor/backtest/hash see the
  // text that would actually mint. Non-length structural rejects are untouched (banned token etc.).
  private async maybeCompressLengthReject(
    revision: { readonly playbook: string; readonly changelog: string },
    gate: ReflectionDraftGate,
    current: { readonly version: number; readonly content: string },
    recordUsage: (usage: AgentUsage | undefined) => void,
  ): Promise<{
    readonly revision: { readonly playbook: string; readonly changelog: string };
    readonly gate: ReflectionDraftGate;
  }> {
    if (
      gate.kind !== 'reject' ||
      !gate.structural ||
      gate.validation === undefined ||
      !isPlaybookLengthReject(gate.validation)
    ) {
      return { revision, gate };
    }
    const before = revision.playbook.length;
    const compressed = compressPlaybookToMaxChars(revision.playbook, MAX_PLAYBOOK_CHARS);
    if (!compressed.ok) {
      this.warn(
        `reflection: length-compress failed (${compressed.reason}) — leaving length reject`,
      );
      return { revision, gate };
    }
    const next = { playbook: compressed.content, changelog: revision.changelog };
    this.warn(
      `reflection: length-compressed playbook ${before}→${compressed.content.length} chars (trimmed ${compressed.trimmedChars}) — re-gating`,
    );
    const nextGate = await this.gateReflectionDraft(next, current, recordUsage);
    // When compress clears length but a later gate mode still rejects, the retry's assistant
    // turn still carries the original over-cap tool_use. Inject the compressed text into
    // feedback so the model fixes the draft that actually failed floor/backtest/structure.
    if (nextGate.kind === 'reject') {
      return {
        revision: next,
        gate: {
          ...nextGate,
          feedback:
            `${nextGate.feedback}\n\n` +
            `NOTE: your submitted playbook exceeded ${MAX_PLAYBOOK_CHARS} chars and was ` +
            `server-compressed to the text below before this check. Revise FROM THIS TEXT ` +
            `(not your original tool_use), keeping ≤${MAX_PLAYBOOK_CHARS} chars:\n` +
            compressed.content,
        },
      };
    }
    return { revision: next, gate: nextGate };
  }
  ...
  private async runMintFloor(
    ...
    // Atomic batch reservation for every replay row BEFORE any replay call: a shortfall means the
    // whole floor aborts (fail-open) rather than judging a candidate on a partial, budget-truncated
    // sample or leaving phantom slots from a partial loop.
    if (!this.spendGate().tryReserveCalls(rows.length)) {
      this.deps.recorder?.recordReflectionOutcome?.('mint_floor_budget_bypass');
  ...
  private async runMintBacktest(
    ...
    // Budget: (2 arms, or 1 with a cached champion) × rows.length calls, atomically reserved up front
    // (fail-open on any shortfall) — same all-or-nothing discipline as runMintFloor.
    const callsNeeded = rows.length * (cache ? 1 : 2);
    if (!this.spendGate().tryReserveCalls(callsNeeded)) {
      this.deps.recorder?.recordReflectionOutcome?.('mint_backtest_budget_bypass');
      this.warn(
