...
// Trivial Map-backed impl of PlanStopRegistryPort (ports/trading/risk.ts) — a single shared instance bridges
// AgenticStrategy (writer, on plan-entry-fill/clear) and ProtectiveExitService (reader, each 1s tick).
// See ports/trading/risk.ts's own header comment for the key convention and venueStopResting's future-proofing.
@Injectable()
