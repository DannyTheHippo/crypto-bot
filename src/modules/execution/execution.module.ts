...
import {
  EXECUTION_GATE, PORTFOLIO_VIEW, EXEC_OUTBOX, EXECUTION_STORE, EXEC_RUN_CONTEXT,
  PORTFOLIO_CONFIG, EXEC_FILTERS, EXEC_REPORT_NOTIFY, EQUITY_OBSERVER, EQUITY_LIMITS,
  RECON_CONFIG, INSTANCE_LOCK, EXEC_OUTBOX_OVERRIDE, EXECUTION_STORE_OVERRIDE, INSTANCE_LOCK_OVERRIDE,
  type ExecOutboxPort, type ExecutionStorePort, type InstanceLockPort,
  type ExecRunContext, type PortfolioConfig, type ExecFilters, type EquityLimits, type ReconConfig,
  type EquityObserver,
...
import {
  FillIngestorService, FILLS_COUNTER,
  ORDERS_FILLED_QTY_COUNTER, ORDERS_FULLY_FILLED_COUNTER, SLIPPAGE_DECISION_HISTOGRAM,
} from './fill-ingestor.service';
import {
  ExecutionGateService, ORDERS_COUNTER, ORDERS_REJECTED_COUNTER,
  ORDERS_SUBMITTED_COUNTER, ORDERS_SUBMITTED_QTY_COUNTER, ORDER_SUBMIT_LATENCY,
...
import {
  ReconciliationService, RECON_MISMATCH_COUNTER, RECON_RUNS_COUNTER, RECON_LAST_SUCCESS_GAUGE,
...
const DEFAULT_RECON_CONFIG: ReconConfig = { epsAbs: '0.00000001', epsRel: '0.0001', overlapMs: 300_000, driftPasses: 3 };
...
const providers: Provider[] = [
  ...
  },
  {
    ...
    useFactory: (config?: ConfigService<AppConfig, true>): PortfolioConfig => portfolioConfigFrom(config),
    inject: [CONFIG_OPTIONAL],
  },
  ...
  },
  {
    ...
    useFactory: (override?: ExecutionStorePort): ExecutionStorePort => override ?? new InMemoryExecutionStore(),
    inject: [{ token: EXECUTION_STORE_OVERRIDE, optional: true }],
  },
  {
    provide: INSTANCE_LOCK,
    useFactory: (override?: InstanceLockPort): InstanceLockPort => override ?? new InMemoryInstanceLock(),
  ...
  DemoFillPollerService,
  {
    ...
    useFactory: (consumer: ExecReportConsumerService) => async () => { await consumer.pump(); },
  ...
  },
  {
    ...
    useFactory: (monitor: EquityMonitorService): EquityObserver => (sample) => { monitor.onSample(sample); },
...
@Module({
  providers,
  exports: [
    EXECUTION_GATE, PORTFOLIO_VIEW, EXEC_OUTBOX, EXEC_REPORT_NOTIFY, EXECUTION_STORE, CLOCK,
    EXEC_FILTERS, FEED_HEALTH, INSTANCE_LOCK,
    ExecReportConsumerService, PortfolioStateService, UnknownResolverService, ReconciliationService,
    CrashRecoveryService, BootRecoveryService, EquitySamplerService,
