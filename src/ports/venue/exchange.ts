...
export interface ExchangePort {
  ...
  // boot call no-ops via `?.`). Implementations are fail-closed — a pin failure throws; the
  // runtime's startTrading().catch engages the kill switch (HALT+flatten) rather than letting
  // an unhandledRejection restart-loop the process.
