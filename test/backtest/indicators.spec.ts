...
//
// Indicator values are plain floats by design (never money — see indicators.ts's header), so the
// float-tolerance comparisons below are legitimate; the repo's blanket `toBeCloseTo`-ban lint rule
// (scoped to money-assertion discipline, CLAUDE.md rule 1) is syntactic and can't distinguish that, so
// this helper reproduces toBeCloseTo's exact semantics (|actual − expected| < 0.5 × 10^−precision)
// without the banned matcher name.
...
describe('research indicators — known inputs', () => {
  it('sma / rollingMeanStd / zscore', () => {
    expectCloseTo(sma([1, 2, 3, 4, 5], 5), 3, 12);
    expect(sma([1, 2], 5)).toBeNaN(); // not enough data
    expectCloseTo(sma([9, 9, 1, 2, 3, 4, 5], 5), 3, 12); // trailing window only
    const { mean, std } = rollingMeanStd([1, 2, 3, 4, 5], 5);
    expectCloseTo(mean, 3, 12);
    expectCloseTo(std, Math.SQRT2, 12); // population std of [1..5]
    // z of the last point: (5−3)/√2
    expectCloseTo(zscore([1, 2, 3, 4, 5], 5), (5 - 3) / Math.SQRT2, 12);
  });

  it('bollinger bands sit symmetric around the SMA mid', () => {
    const b = bollinger([1, 2, 3, 4, 5], 5, 2);
    expectCloseTo(b.mid, 3, 12);
    expectCloseTo(b.upper, 3 + 2 * Math.SQRT2, 12);
    expectCloseTo(b.lower, 3 - 2 * Math.SQRT2, 12);
  });
  ...
  it('ATR equals the constant true range of a steady series', () => {
    ...
    expectCloseTo(atr(highs, lows, closes, 14), 2, 10);
  ...
  it('momentum is the trailing return', () => {
    expectCloseTo(momentum([100, 100, 110], 2), 0.1, 12); // 110/100 − 1
