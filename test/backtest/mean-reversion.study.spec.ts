...
describe('mean-reversion out-of-sample study (real Binance BTC/USDT)', () => {
  it('runs the grid across intervals, splits IS/OOS, and writes the report', () => {
    ...
    lines.push('');
    lines.push(
      `Real Binance BTC/USDT OHLCV (same cached datasets as the EMA study). Fee = ${FEE_BPS} bps/side ` +
    ...
    lines.push('');
    lines.push(
      `> **Hypothesis:** short-horizon liquidity-provision / overreaction reversal — buy transient weakness ` +
    ...
    for (const interval of INTERVALS) {
      ...
      for (const row of byIs) {
        lines.push(
          `| ${row.lookback}/${row.entryZ.toFixed(1)} | ${fmt(row.is.returnPct)} | ${row.is.trades} | ${(row.is.winRate * 100).toFixed(0)} | ` +
            `${fmt(row.oos.returnPct)} | ${row.oos.trades} | ${(row.oos.winRate * 100).toFixed(0)} |`,
        );
      }
      lines.push('');
      lines.push(
        `- **IS-best** ${isBest.lookback}/${isBest.entryZ.toFixed(1)}: IS ${fmt(isBest.is.returnPct)}% → **OOS ${fmt(isBest.oos.returnPct)}%** ` +
          `(${isBestSurvives ? 'survives OOS' : 'FAILS OOS'})`,
      );
      lines.push(
        `- Best OOS combo ${oosBest.lookback}/${oosBest.entryZ.toFixed(1)}: OOS ${fmt(oosBest.oos.returnPct)}% (its IS ${fmt(oosBest.is.returnPct)}%)`,
      );
      lines.push(
      ...
      summary.push(
        `${interval}: IS-best ${isBest.lookback}/${isBest.entryZ.toFixed(1)} IS ${fmt(isBest.is.returnPct)}% → OOS ${fmt(isBest.oos.returnPct)}% ` +
    ...
    console.log(
      `positive-both total: ${totalPosBoth}/${nThisStudy} (@${FEE_BPS}bps), ${totalPosBoth0}/${nThisStudy} (@0bps)`,
    );
