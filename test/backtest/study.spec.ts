...
describe('EMA-cross out-of-sample parameter study (real Binance BTC/USDT)', () => {
  it('runs the grid across intervals, splits IS/OOS, and writes the report', () => {
    ...
    lines.push('');
    lines.push(
      `Real Binance BTC/USDT OHLCV. Fee = ${FEE_BPS} bps/side (taker, conservative; ~${FEE_BPS * 2} bps round-trip). ` +
    ...
    lines.push('');
    lines.push(
      '> **Validated** by a 5-skeptic adversarial methodology audit (0 critical, 0 verdict-flippers): ' +
    ...
    for (const interval of INTERVALS) {
      ...
      for (const row of byIs) {
        lines.push(
          `| ${row.fast}/${row.slow} | ${fmt(row.is.returnPct)} | ${row.is.trades} | ${(row.is.winRate * 100).toFixed(0)} | ` +
            `${fmt(row.oos.returnPct)} | ${row.oos.trades} | ${(row.oos.winRate * 100).toFixed(0)} |`,
        );
      }
      lines.push('');
      lines.push(
        `- **IS-best** ${isBest.fast}/${isBest.slow}: IS ${fmt(isBest.is.returnPct)}% → **OOS ${fmt(isBest.oos.returnPct)}%** ` +
          `(${isBestSurvives ? 'survives OOS' : 'FAILS OOS'})`,
      );
      lines.push(
        `- Best OOS combo ${oosBest.fast}/${oosBest.slow}: OOS ${fmt(oosBest.oos.returnPct)}% (its IS ${fmt(oosBest.is.returnPct)}%)`,
      );
      lines.push(
        `- Live default 9/21: IS ${fmt(r.rows.find((x) => x.fast === 9 && x.slow === 21)?.is.returnPct ?? 0)}% · ` +
          `OOS ${fmt(r.rows.find((x) => x.fast === 9 && x.slow === 21)?.oos.returnPct ?? 0)}%`,
      );
      lines.push(
        `- Positive: IS ${posIs}/${r.rows.length} · OOS ${posOos}/${r.rows.length} · **BOTH ${posBoth}/${r.rows.length}** · ` +
          `**BOTH @ 0 bps (no fees): ${r.posBoth0}/${r.rows.length}**`,
      );
      lines.push('');

      summary.push(
        `${interval}: IS-best ${isBest.fast}/${isBest.slow} IS ${fmt(isBest.is.returnPct)}% → OOS ${fmt(isBest.oos.returnPct)}% ` +
          `[${isBestSurvives ? 'survives' : 'fails'}]; positive-both ${posBoth}/${r.rows.length} (0bps ${r.posBoth0}/${r.rows.length}); B&H OOS ${fmt(r.oosBH)}%`,
      );
    }

    const totalPosBoth0 = INTERVALS.map((iv) => studyInterval(iv)?.posBoth0 ?? 0).reduce(
      (a, b) => a + b,
      0,
    );
    lines.push('## Verdict');
