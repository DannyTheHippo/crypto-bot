...
describe('DailyLlmBudget', () => {
  ...
  it('tryReserveCalls(0) succeeds without changing state', () => {
    const budget = new DailyLlmBudget({ maxCallsPerDay: 1, maxTokensPerDay: 1_000_000 }, () => T);
    expect(budget.tryReserveCalls(0)).toBe(true);
    expect(budget.snapshot().calls).toBe(0);
  });

  it('tryReserveCalls rejects invalid counts', () => {
    const budget = new DailyLlmBudget({ maxCallsPerDay: 10, maxTokensPerDay: 1_000_000 }, () => T);
    expect(() => budget.tryReserveCalls(-1)).toThrow(RangeError);
    expect(() => budget.tryReserveCalls(1.5)).toThrow(RangeError);
    expect(budget.snapshot().calls).toBe(0);
  });

  it('tryReserveCalls is all-or-none — a shortfall leaves no phantom slots', () => {
    const budget = new DailyLlmBudget({ maxCallsPerDay: 5, maxTokensPerDay: 1_000_000 }, () => T);
    expect(budget.tryReserveCalls(4)).toBe(true);
    expect(budget.snapshot().calls).toBe(4);
    expect(budget.tryReserveCalls(2)).toBe(false); // only 1 slot left
    expect(budget.snapshot().calls).toBe(4); // unchanged — no partial reservation
    expect(budget.tryReserveCall()).toBe(true); // the one remaining slot still works
...
describe('XA2: tryReserveAttempt + AttemptScopedBudget', () => {
  ...
  it('AttemptScopedBudget tryReserveCalls is all-or-none across session and daily caps', () => {
    const budget = new DailyLlmBudget({ maxCallsPerDay: 10, maxTokensPerDay: 1_000_000 }, () => T);
    const attempt = new AttemptScopedBudget(budget, 3, 10);
    expect(attempt.tryReserveCalls(2)).toBe(true);
    expect(attempt.snapshot().calls).toBe(2);
    expect(budget.snapshot().calls).toBe(2);
    expect(attempt.tryReserveCalls(2)).toBe(false); // session cap is 3, only 1 left
    expect(attempt.snapshot().calls).toBe(2); // no phantom slot on the daily pool either
    expect(budget.snapshot().calls).toBe(2);
    expect(attempt.tryReserveCall()).toBe(true);
    expect(attempt.snapshot().calls).toBe(3);
    expect(budget.snapshot().calls).toBe(3);
  });
});
