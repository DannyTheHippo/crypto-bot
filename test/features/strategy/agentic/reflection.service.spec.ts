...
import type { KillSwitchState } from '../../../../src/domain/trading/risk/kill-switch';
...
} from '../../../../src/features/strategy/agentic/reflection.service';
import type {
...
describe('ReflectionService', () => {
  ...
  describe('backlog #31: validator-reject retry-with-feedback + additive trigger rollback', () => {
    ...
    it('retries once with feedback after a first-draft validator rejection, then mints on the accepted retry (2 fetch calls, 2 budget reservations)', async () => {
      ...
      const reserveSpy = vi.spyOn(h.budget, 'tryReserveCalls');
      const service = new ReflectionService(baseCfg({ everyNTrades: 1 }), h.deps);
      ...
      expect(reserveSpy).toHaveBeenNthCalledWith(1, 1);
      expect(reserveSpy).toHaveBeenNthCalledWith(2, 1);
      expect(h.storeApi.appended).toEqual([
      ...
      expect(toolResultBlock['content']).toContain('HARD CAP ≤4000 characters');
    });

    it('length-compresses an over-cap first draft and mints without a second Opus call', async () => {
      // 2026-07-24: live drafts landed 16–283 chars over; Opus retry stayed over. Section-aware
      // compress must recover before burning the bounded LLM retry.
      const overCap = [
        '## regime notes',
        'notes overflow',
        '## entry rules',
        ...
        'exit overflow',
        '## mistakes to avoid',
        `mistakes overflow ${'PAD. '.repeat(900)}`,
      ].join('\n');
      expect(overCap.length).toBeGreaterThan(4000);

      const h = buildHarness();
      h.fetchFn.mockResolvedValueOnce(
        apiResponse(revisionToolBody({ playbook: overCap, changelog: 'anti-ratchet note' })),
      );
      const service = new ReflectionService(baseCfg({ everyNTrades: 1 }), h.deps);
      ...
      expect(h.fetchFn).toHaveBeenCalledTimes(1); // no LLM retry
      expect(h.storeApi.appended).toHaveLength(1);
      expect(h.storeApi.appended[0]!.source).toBe('reflection');
      expect(h.storeApi.appended[0]!.content.length).toBeLessThanOrEqual(4000);
      expect(h.storeApi.appended[0]!.content).toContain('## mistakes to avoid');
      expect(h.recorderApi.outcomes).toEqual(['attempt_started', 'minted']);
      expect(h.logger.messages.some((m) => m.includes('length-compressed playbook'))).toBe(true);
    });
...
describe('X8: per-version net-PnL digest in the reflection payload', () => {
  ...
  it('surfaces the 4000-char hard cap in the system prompt and submit_playbook_revision tool schema', async () => {
    // 2026-07-23: Opus drafted 4523 chars against a ceiling-sitting seed; the reject reason named
    // 4000 but the first-shot prompt/tool never did — pin the control plane here.
    const h = buildHarness();
    h.fetchFn.mockResolvedValue(
      apiResponse(revisionToolBody({ playbook: validPlaybookContent('v2'), changelog: 'c' })),
    );
    ...
    const body = requestBodyOf(h.fetchFn) as unknown as {
      system: string;
      tools: Array<{
        name: string;
        input_schema: { properties: { playbook: { description: string } } };
      }>;
    };
    expect(body.system).toContain('HARD LENGTH CAP');
    expect(body.system).toContain('≤4000 characters');
    const playbookTool = body.tools.find((t) => t.name === 'submit_playbook_revision');
    expect(playbookTool?.input_schema.properties.playbook.description).toContain(
      'HARD CAP ≤4000 characters',
    );
  });
});
...
describe('backlog #39: mint-time entry-rate floor', () => {
  ...
  it('fails open (mint proceeds) when the daily budget is exhausted mid-floor-replay', async () => {
    const h = buildHarness({
      // attempt_started's own reservation already spends the only allowed call — the floor's atomic
      // batch reservation then fails immediately.
