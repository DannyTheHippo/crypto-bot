...
import { validateLimits, type PartialRiskLimits } from '../../../../src/domain/trading/risk/limits';
...
describe('validateLimits (§5 G2)', () => {
  ...
  it('rejects a missing maxStopTriggerBandBps', () => {
    expect(validateLimits({ ...VALID, maxStopTriggerBandBps: null })).toBeNull();
  });
  it('rejects a missing stopLimitBufferBps', () => {
    expect(validateLimits({ ...VALID, stopLimitBufferBps: null })).toBeNull();
  });
