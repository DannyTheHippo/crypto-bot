...
function build(over: Partial<ModeControlConfig> = {}) {
  ...
  const cfg: ModeControlConfig = { requested: 'paper', bootId: BOOT, armingSecret: SECRET, limitsComplete: true, ...over };
  const svc = new ModeControlService(clock, keyProbe, killSwitch, audit, preconditions, cfg);
  return {
    svc, events, killSwitch,
    setNow: (n: number) => { t = n; }, setProbe: (p: KeyProbeResult) => { probe = p; }, setPrecond: (p: ArmPreconditionResult) => { precond = p; },
  };
}
...
describe('LIVE-GATE MATRIX — 2⁴ effective-mode truth table', () => {
  ...
  for (const env of axes) for (const armed of axes) for (const keys of axes) for (const limits of axes) {
    ...
    it(`env=${env} armed=${armed} keys=${keys} limits=${limits} → ${expected}`, async () => {
      ...
      if (keys) { ctx.setProbe(validProbe); await ctx.svc.refreshKeyProbe(); }
      if (armed) arm(ctx);
...
describe('LIVE-GATE MATRIX — arming interlock safety rows', () => {
  it('S3 restart: a confirm bound to a different bootId is refused (captured-token defence)', () => {
    ...
    const req = ctx.svc.armLive({ step: 'REQUEST', bootId: BOOT });
    const proofForOtherBoot = computeArmingHmac((req as { challengeId: string }).challengeId, 'boot-2', SECRET);
    const res = ctx.svc.armLive({ step: 'CONFIRM', challengeId: (req as { challengeId: string }).challengeId, hmacHex: proofForOtherBoot, bootId: 'boot-2' });
