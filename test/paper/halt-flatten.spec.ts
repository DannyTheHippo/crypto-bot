...
function entryIntent(q: string): OrderIntent {
  ...
  return {
    intentId: id, clientOrderId: encodeClientOrderId(id, 'paper'), strategyId: SID, venue: V, symbol: SYM,
    side: 'BUY', type: 'LIMIT', qty: qty(q), limitPrice: price('100'), timeInForce: 'GTC', reduceOnly: false,
    mode: 'paper', refPrice: price('100'), refSeq: 1n, createdAt: epochMs(T), expiresAt: epochMs(T + 10_000),
...
function build() {
  ...
  const feed: FeedHealthPort = {
    getRefPrice: () => ({ mid: price(mark), at: epochMs(T) }), health: () => 'LIVE', fetchCandles: () => Promise.resolve([]),
  };
  ...
  const paperCfg: PaperConfig = {
    seed: 1, takerBuffer: '0.05', fees: { makerBps: '1', takerBps: '10', feeCurrency: 'quote' },
  ...
  const engine = new RiskEngineService(
    clock, { key: KEY, limits: RISK_LIMITS, limitsVersion: 'v1', mode: 'paper', filters: FILTERS, randomBytes: (n) => new Uint8Array(n).fill(3) },
  ...
  return { clock, killSwitch, portfolio, sampler, coord, orders, adapter, setMark: (m: string) => { mark = m; } };
}
...
describe('integrated drawdown → HALTING → FLATTENING → flat (real adapter, real fill)', () => {
  it('flattens a long to flat through a real fill and reaches HALTED with a clean book', async () => {
    ...
    ctx.portfolio.applyFill(entryIntent('100'), {
      venue: V, symbol: SYM, venueTradeId: 'entry', clientOrderId: entryIntent('100').clientOrderId,
      price: price('100'), qty: qty('100'), fee: null, liquidity: 'taker', venueTimestamp: epochMs(T), source: 'paper',
    });
