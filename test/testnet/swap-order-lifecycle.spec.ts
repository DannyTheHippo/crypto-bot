...
// resolveVenueUrls reads only urls.api/hostname; ccxt's Exchange types urls.api as a looser
// string | Dictionary union, so narrow via the same cast the sandbox-url regression spec uses.
function swapPrivateUrl(exchange: Exchange): string {
  return resolveVenueUrls(
    exchange as unknown as { urls: { api?: Record<string, unknown> }; hostname?: string },
    'binanceusdm',
  ).restSwapPrivate;
}
...
describe.skipIf(!HAS_CREDS)('Binance USD-M swap (binanceusdm) Demo Trading lifecycle', () => {
  it('0. resolves the private fapi base URL to demo-fapi.binance.com BEFORE any order call (fail-closed)', () => {
    ...
    const restSwapPrivate = swapPrivateUrl(exchange);

    expect(new URL(restSwapPrivate).hostname).toBe('demo-fapi.binance.com');
    expect(() => assertSwapPrivateUrlSafe(restSwapPrivate, 'demo')).not.toThrow();
  });
  ...
  it('1. mark price, deep-OTM resting LIMIT buy at venue-minimum qty, fetchOpenOrders sees it, cancel', async () => {
    ...
    assertSwapPrivateUrlSafe(swapPrivateUrl(exchange), 'demo');
