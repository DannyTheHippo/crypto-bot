...
function clientThrowing(): CcxtOrderClient {
  return stubClient({ sapiGetAccountApiRestrictions: vi.fn().mockRejectedValue(new Error('NotSupported')) });
}
function stubClient(over: Partial<CcxtOrderClient>): CcxtOrderClient {
  return {
    createOrder: vi.fn(), cancelOrder: vi.fn(), fetchOrder: vi.fn(), fetchOpenOrders: vi.fn(),
    fetchBalance: vi.fn(), fetchMyTrades: vi.fn(), sapiGetAccountApiRestrictions: vi.fn().mockResolvedValue({}),
    ...over,
  };
}
...
describe('KeyProbeService', () => {
  it('a withdrawals-disabled, spot-enabled, no-margin/futures key is valid', async () => {
    const r = await probeWith(clientReturning({
      enableWithdrawals: false, enableSpotAndMarginTrading: true, enableMargin: false, enableFutures: false,
    })).probe();
    expect(r).toEqual({ keysValid: true, withdrawalsEnabled: false, spotEnabled: true, marginOrFutures: false, keyFingerprint: FP, urlCrossCheckOk: true });
  });

  it('a withdrawals-ENABLED key is invalid (cardinal refusal) with the flag surfaced', async () => {
    const r = await probeWith(clientReturning({
      enableWithdrawals: true, enableSpotAndMarginTrading: true, enableMargin: false, enableFutures: false,
    })).probe();
  ...
  it('a key without spot trading is invalid', async () => {
    const r = await probeWith(clientReturning({
      enableWithdrawals: false, enableSpotAndMarginTrading: false, enableMargin: false, enableFutures: false,
    })).probe();
    expect(r.keysValid).toBe(false);
  ...
  it('a margin- or futures-enabled key is invalid (marginOrFutures flagged)', async () => {
    const margin = await probeWith(clientReturning({
      enableWithdrawals: false, enableSpotAndMarginTrading: true, enableMargin: true, enableFutures: false,
    })).probe();
    expect(margin).toMatchObject({ keysValid: false, marginOrFutures: true });
    const futures = await probeWith(clientReturning({
      enableWithdrawals: false, enableSpotAndMarginTrading: true, enableMargin: false, enableFutures: true,
    })).probe();
  ...
  it('an unprobeable restriction set (testnet NotSupported) degrades to valid when restrictions are NOT required', async () => {
    ...
    expect(r).toMatchObject({ keysValid: true, withdrawalsEnabled: false, spotEnabled: true, keyFingerprint: FP });
  });
