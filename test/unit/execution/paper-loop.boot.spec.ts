...
import { PaperExchangeAdapter, PAPER_CONFIG, type PaperConfig } from '../../../src/modules/exchange-adapter/paper-exchange.adapter';
...
import {
  EXECUTION_GATE, PORTFOLIO_VIEW, EXEC_OUTBOX, EXEC_REPORT_NOTIFY,
  type ExecOutboxPort, type ExecReportNotify,
...
@Module({
  ...
  providers: [
    { provide: PAPER_CONFIG, useValue: PAPER_CFG },
    {
      ...
      useFactory: (clock: ClockPort, outbox: ExecOutboxPort, notify: ExecReportNotify, cfg: PaperConfig) =>
...
describe('paper loop DI boot', () => {
  it('resolves the trading module graph (no DB, no token mismatches)', async () => {
    const ref = await Test.createTestingModule({
      imports: [KeyModule, KillSwitchModule, ModeControlStubModule, RiskModule, ExecutionModule, PaperExchangeModule],
      providers: [SignalSinkService],
    })
      .overrideProvider(CLOCK).useValue({ now: () => epochMs(T) })
      .overrideProvider(FEED_HEALTH).useValue(fedFeed)
      .compile();
