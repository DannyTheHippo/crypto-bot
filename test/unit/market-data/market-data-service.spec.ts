...
import type { ExchangeStreamPort, RawVenueEvent, RawUserEvent } from '../../../src/ports/exchange-stream';
...
describe('MarketDataService.subscribe', () => {
  it('normalizes raw events and stamps ingestTime from the clock', async () => {
    const raws: RawVenueEvent[] = [
      { type: 'ticker', venue: V, symbol: S, timestamp: epochMs(1), raw: { timestamp: 1, bid: '100', ask: '101', last: '100.5' } },
      { type: 'trade', venue: V, symbol: S, timestamp: epochMs(2), raw: { timestamp: 2, price: '100', amount: '1', side: 'sell', id: 'x' } },
    ];
    const svc = new MarketDataService(streamOf(raws), clock);
    const spec: SubscriptionSpec = { venue: V, symbols: [S], channels: { ticker: true, trades: true } };
  ...
  it('skips an event that fails normalization without killing the stream', async () => {
    const raws: RawVenueEvent[] = [
      ...
      { type: 'ticker', venue: V, symbol: S, timestamp: epochMs(2), raw: { timestamp: 2, ask: '101', last: '100.5' } },
      { type: 'trade', venue: V, symbol: S, timestamp: epochMs(3), raw: { timestamp: 3, price: '100', amount: '1', side: 'buy', id: 'y' } },
    ];
    const svc = new MarketDataService(streamOf(raws), clock);
    const spec: SubscriptionSpec = { venue: V, symbols: [S], channels: { ticker: true, trades: true } };
    const events = await collect(svc.subscribe(spec));
  ...
  it('passes the subscription candle interval through to normalization', async () => {
    const raws: RawVenueEvent[] = [
      { type: 'candle', venue: V, symbol: S, timestamp: epochMs(60000), raw: [60000, '100', '110', '90', '105', '10', true] },
    ];
    const svc = new MarketDataService(streamOf(raws), clock);
