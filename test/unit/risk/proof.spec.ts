...
import { mintApproval, verifyApproval, APPROVAL_TTL_MS, type MintOptions } from '../../../src/domain/risk/proof';
...
function intent(o: Partial<OrderIntent> = {}): OrderIntent {
  return {
    intentId: IID, clientOrderId: encodeClientOrderId(IID, 'paper'), strategyId: strategyId('s'),
    venue: venueId('binance'), symbol: symbolId('BTC/USDT'), side: 'BUY', type: 'LIMIT',
    qty: qty('1'), limitPrice: price('100'), timeInForce: 'GTC', reduceOnly: false, mode: 'paper',
    refPrice: price('100'), refSeq: 1n, createdAt: epochMs(0), expiresAt: epochMs(10),
    source: { dedupeKey: 'k', eventTime: epochMs(0), basedOnSeq: 1n, strength: 1 }, ...o,
  };
}

const OPTS: MintOptions = { nonce: 'n1', approvedAtMs: epochMs(1000), limitsVersion: 'v1', snapshotSeq: 5n };
...
describe('approval proof (§4.2 unforgeable RiskApprovedIntent)', () => {
  ...
  it('a mutated intent fails the hash check', () => {
    const approved = mintApproval(intent(), KEY, OPTS);
    const tampered = { ...approved, intent: { ...approved.intent, qty: qty('999') } } as typeof approved;
  ...
  it('a stale approval (past TTL) is refused', () => {
    const approved = mintApproval(intent(), KEY, OPTS);
    expect(verifyApproval(approved, KEY, epochMs(1000 + APPROVAL_TTL_MS + 1), false)).toBe('EXPIRED');
  });

  it('the same intent + nonce + key produces a stable hmac (deterministic)', () => {
    expect(mintApproval(intent(), KEY, OPTS).proof.hmac).toBe(mintApproval(intent(), KEY, OPTS).proof.hmac);
  });
});
