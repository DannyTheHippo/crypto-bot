...
const sizerDeps: SizerDeps = { baseNotional: '1000', mode: 'paper', filters: new Map([[String(SYM), FILTERS]]), randomBytes: (n) => new Uint8Array(n).fill(3) };
const engineDeps: RiskEngineDeps = { key: Buffer.alloc(32, 1), limits: LIMITS, limitsVersion: 'v1', mode: 'paper', filters: new Map([[String(SYM), FILTERS]]), randomBytes: (n) => new Uint8Array(n).fill(3) };
const feed: FeedHealthPort = { getRefPrice: () => ({ mid: price('100'), at: epochMs(T) }), health: () => 'LIVE', fetchCandles: () => Promise.resolve([]) };

function signal(o: Partial<Signal> = {}): Signal {
  return {
    strategyId: SID, venue: V, symbol: SYM, kind: 'ENTER_LONG', strength: 1, refPrice: price('100'),
    basedOnSeq: 1n, eventTime: epochMs(T), ttlMs: 5000, dedupeKey: 'k1', reason: 'r', ...o,
  };
}
...
function makeGateway() {
  ...
  const engine = new RiskEngineService(clock, engineDeps, feed, kill, new RateBucketsService(clock), new CrossingRegistryService(), journal);
...
describe('SignalGatewayService', () => {
  ...
  it('fast-fails when the kill switch is engaged', () => {
    ...
    expect(gateway.accept(signal(), snapshot())).toEqual({ status: 'GATEWAY_REJECTED', reason: 'KILL_SWITCH' });
  });
  ...
  it('drops a duplicate dedupeKey', () => {
    ...
    expect(gateway.accept(signal(), snapshot())).toEqual({ status: 'GATEWAY_REJECTED', reason: 'DUPLICATE' });
  });
  ...
  it('evicts dedupe keys past their TTL window while retaining live ones (bounded memory)', () => {
    ...
    const kill = new KillSwitchService();
    const journal: RiskJournalPort = { record: () => undefined };
    const engine = new RiskEngineService(mclock, engineDeps, feed, kill, new RateBucketsService(mclock), new CrossingRegistryService(), journal);
    ...
    t = T + 5000;
    gateway.accept(signal({ dedupeKey: 'fresh', eventTime: epochMs(T + 5000), ttlMs: 5000 }), snapshot());
    ...
    expect(gateway.accept(signal({ dedupeKey: 'long', eventTime: epochMs(T), ttlMs: 60_000 }), snapshot()))
